...
const SideNavbar = () => {
  ...
  return (
    <div className="w-full h-full">
      ...
      <div
        className={`${navbarScroll} ${isOpen ? "max-md:hidden" : "max-md:flex fixed top-0 left-0 right-0"} hidden w-full z-[999999] max-md:items-center py-2 max-md:justify-around`}
      ...
      {/* Side Navbar */}
      <div
        className={`fixed top-0 rounded-tl-[15px] right-0 h-full overflow-hidden z-[99999] ${isOpen ? "w-64 max-md:w-full" : " "} bg-[#5650ce] text-white shadow-lg transition-transform duration-300 ease-in-out transform ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="flex flex-col h-full">
          {/* Profile Section */}
          <div className="flex items-center w-[90%] rounded-[15px] mx-auto h-fit mt-4 justify-center bg-[#181a3da9] shadow-lg">
            <div className="fixed top-3 z-[999999] left-2 p-4" onClick={toggleSidebar}>
              {isOpen ? (
            ...
            {user && (
              <div className="h-full w-full flex flex-col items-center p-4">
                <div className="relative w-[70px] flex items-center flex-col justify-center ">
                  <img
                    src={user.profilePicture || husband}
                    alt="Profile"
                    className="h-[60px] w-[60px] rounded-full border-2 border-white"
                  />
                  <div className={`absolute top-0 right-2 w-3 h-3 rounded-full ${user.isOnline ? "bg-green-500" : "bg-red-500"}`} />
                </div>
                <h3 className="w-full text-center font-bold text-lg mt-2">{user.name || "محمد محمود"}</h3>
                <div className="flex items-center justify-center gap-2 mt-2">
                  <span className="flex gap-1 items-center">
                    <GiAges />
                    <p className="text-sm text-gray-300">{user.age || "30"} سنة</p>
                  </span>
                  <span className="flex gap-1 items-center">
                    <BsGenderMale className="bg-[#5650ce] rounded p-1" />
                    <p className="text-sm text-gray-300">{user.gender === "male" ? "ذكر" : "ذكر"}</p>
                  </span>
                </div>
                <div className="flex items-center justify-around w-full mt-4">
                  ...
                  <p onClick={() => Logout(false)} className="text-sm flex items-center gap-1 text-gray-300 hover:text-white cursor-pointer">
